...
| 3 | Commit in the workspace, then click **Send to Windows**: `wsl -d willie --user willie -- sh -lc "cd ~/projects/<slug> && echo hi > a.txt && git add -A && git commit -m agent"` | the row settles to `ready`; `git -C <win> show --stat HEAD` shows the commit and `a.txt` now in the checkout's working tree |
