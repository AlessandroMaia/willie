...
| 19 | In a plain `wsl -d willie --user willie` shell: `ln -s /etc /home/willie/projects/<slug>/looks-safe`; then Sandbox… on the row → add `extra.paths` `/home/willie/projects/<slug>/looks-safe` read-only → open a session on that project | the dialog accepts the entry — the path named is not itself guarded; opening the session fails instead, with `sandbox_profile_invalid` naming the path it resolves to (`/etc`) — the guard is textual, so a symbolic link into a guarded location is only caught when the resolved path is checked again at launch, before anything is mounted |
