...
| 22 | With that resumed session still live, click **Resume** on `<proj>` again | the row shows an inline `session_already_live` problem with the remediation "use the running session, or stop it first, then resume", and no new session is created |
